...
module.exports = NodeHelper.create({
    ...
    filterGameByFocus(game) {
        ...
        // No focus list configured -> never filter
        if (!Array.isArray(focus) || focus.length === 0) {
            return true;
        }

        // � if false, do NOT filter (show all games)
        if (this.config.filter_on_focus_team === false) {
            return true;
        }
        ...
        // If mapping isn't available for some reason, fail open (show game)
        if (!homeTeam || !awayTeam) {
            return true;
        }

        // Original behavior: keep only focus-team games
        return focus.includes(homeTeam) || focus.includes(awayTeam);
    },

    /**
     * @function isFocusGame
     * @description Helper function to determine if a game involves a focus team.
     *              This is used for styling (e.g., bold text) when displaying all games.
     *
     * @param {object} game - Raw game information.
     *
     * @returns {boolean} True if focus team is participating in the game, otherwise false.
     */
    isFocusGame(game) {
        const focus = this.config.focus_on;
        if (!Array.isArray(focus) || focus.length === 0) {
    ...
    async updateSchedule() {
        ...
        const games = focusSchedule.map((rawGame) => {
            const parsed = this.parseGame(rawGame);
            parsed.isFocusGame = this.isFocusGame(rawGame); // ✅ adds flag for template styling
            return parsed;
        });
